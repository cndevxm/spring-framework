 ...
 * <p>
 * 处理请求流程如下：
 * 1、搜索WebApplicationContext 并将其绑定在DispatcherServlet.WEB_APPLICATION_CONTEXT_ATTRIBUTE上
 * 2、为请求绑定local resolver 以处理请求时的区域设置
 * 3、为请求绑定主题解析器
 * 4、如果指定了multipart file resolver，则将请求包装为MultipartHttpServletRequest
 * 5、搜索适当的程序处理器，如果找到处理程序，则执行与处理程序相关联的执行链（预处理器、后处理器和控制器）
 * 6、如果返回模型，则呈现视图
 */
